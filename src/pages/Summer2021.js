...
// GraphQL Query
const SummerSeason2021Anime = gql`
...
const SummerSeason2021 = () => {
  const { loading, data, error } = useQuery(SummerSeason2021Anime);

  if (loading) return <span className="loader"></span>;
  if (error) return <p>Error...</p>;
  const SummerAnime = data.Page.media;

  return SummerAnime.map(
    ({
      coverImage: { large },
      title: { userPreferred },
      studios,
...
const Summer2021 = () => {
  useEffect(() => {
    document.title = "AniSchedule: Summer 2021 Seasonal Anime";
  });
  return (
    <div>
      <Header />
