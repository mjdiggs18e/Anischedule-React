...
// GraphQL Query
const SpringSeason2021Anime = gql`
...
const SpringSeason2021 = () => {
  const { loading, data, error } = useQuery(SpringSeason2021Anime);

  if (loading) return <span className="loader"></span>;
  if (error) return <p>Error...</p>;
  const SpringAnime = data.Page.media;

  return SpringAnime.map(
    ({
      coverImage: { large },
      title: { userPreferred },
      studios,
...
const Spring2021 = () => {
  useEffect(() => {
    document.title = "AniSchedule: Spring 2021 Seasonal Anime";
  });
