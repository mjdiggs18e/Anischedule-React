...
// GraphQL Query
const WinterSeason2021Anime = gql`
...
const WinterSeason2021 = () => {
  const { loading, data, error } = useQuery(WinterSeason2021Anime);

  if (loading) return <span className="loader"></span>;
  if (error) return <p>Error...</p>;
  const WinterAnime = data.Page.media;

  return WinterAnime.map(
    ({
      coverImage: { large },
      title: { userPreferred },
      studios,
      ...
      return (
        <AnimeCard key={id}>
          ...
          <AnimeCardRight>
            ...
            ></AnimeCardDescription>
            <AnimeCardGenre>{genres[0]}</AnimeCardGenre>
            <AnimeCardWatch>
...
const Winter2021 = () => {
  useEffect(() => {
    document.title = "AniSchedule: Winter 2021 Seasonal Anime";
  });
  return (
    <div>
      <Header />
